...
import { ACCOUNT_ICON_MAP } from "./helpers/accountAppearance";
...
export default function AddTransaction() {
  ...
  const NUM_COLUMNS = width < 360 ? 2 : width < 768 ? 3 : 4;

  const ITEM_WIDTH = (width - 32 - 12 * (NUM_COLUMNS - 1)) / NUM_COLUMNS;

  const { category: newCategoryParam } = useLocalSearchParams();
  ...
  useEffect(() => {
    ...
    try {
      const parsed = JSON.parse(newCategoryParam);

      if (!parsed?.type || !parsed?.category_id) {
        return;
      }
      ...
      console.log("Error parsing new category:", error);
    }
  }, [newCategoryParam]);
  ...
  const fetchAccounts = useCallback(async () => {
    try {
      ...
      const response = await fetch(`${API_URL}/api/accounts/view`, {
        method: "GET",
      ...
      if (!response.ok) {
        console.log("Accounts API error:", result);
  ...
  const fetchCategoriesByType = useCallback(async (catType) => {
    try {
      const token = await getAuthToken();
      ...
      console.log(`${catType} status:`, response.status);

      if (!response.ok) {
        console.log(`${catType} category error:`, result);

        return [];
      }

      console.log(`${catType} categories:`, result.data?.length || 0);

      return result.data || [];
    } catch (error) {
      console.log(`${catType} request error:`, error.message);
  ...
  const loadAllCategories = useCallback(async () => {
    ...
    try {
      const [expense, income, transfer] = await Promise.all([
        ...
        fetchCategoriesByType("transfer"),
      ]);

      setBackendCategories((prev) => ({
        expense: expense !== null ? expense : prev.expense,

        income: income !== null ? income : prev.income,

        transfer: transfer !== null ? transfer : prev.transfer,
      }));
    } catch (error) {
  ...
  useFocusEffect(
    useCallback(() => {
      const loadData = async () => {
        await Promise.all([fetchAccounts(), loadAllCategories()]);
      };

      loadData();
    }, [fetchAccounts, loadAllCategories]),
  );
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (result.alert) {
        Alert.alert("Budget Alert", result.alert.message);
      }
  ...
  return (
    ...
    >
          ...
          <Box flex={1}>
            <Pressable onPress={() => router.back()}>
              <HStack alignItems="center">
                ...
                <Text ml="$1" fontWeight="$medium" color={colors.text}>
                  Trans.
                </Text>
              </HStack>
            </Pressable>
      ...
      >
        <Box bg={colors.card} p="$4">
          {/* TYPE SELECTOR */}

          <HStack px="$4" mt="$5" space="sm">
              ...
              return (
                ...
                >
                  <Feather
                    ...
                    size={18}
                    color={active ? TYPE_CONFIG[t].color : "#999"}
                  />

                  <Text color={active ? TYPE_CONFIG[t].color : colors.subText}>
                    {TYPE_CONFIG[t].label}
                  </Text>
                </Pressable>
              );
          ...
          <Box mt="$10" alignItems="center">
            <Pressable onPress={() => setShowCalc(true)}>
              <VStack alignItems="center">
                <Text fontSize="$xs" color={colors.subText}>
                  Tap to enter amount
                </Text>

                <Text fontSize="$xl" fontWeight="$bold" color={colors.subText}>
          ...
          <VStack px="$4" mt="$6" space="sm">
            <FormRow label="Date">
              ...
              >
                <Text color={colors.text}>{formatDate(date)}</Text>
              </View>
            </FormRow>
              ...
              <>
                <FormRow label="From">
                  ...
                  >
                    ...
                    >
                      <Text color={colors.text}>{fromAccount || "Select"}</Text>
                    </Pressable>
                  </View>
                </FormRow>
                ...
                <FormRow label="To">
                  ...
                  >
                    ...
                    >
                      <Text color={colors.text}>{toAccount || "Select"}</Text>
                    </Pressable>
                  </View>
                </FormRow>
              </>
              ...
              <>
                <FormRow label="Category">
                  ...
                  >
                    ...
                    >
                      <Text
                        color={
                          category === "Select" ? colors.subText : colors.text
                        }
                      >
                        {category}
                      </Text>
                ...
                <FormRow label="Account">
                  ...
                  >
                    ...
                    >
                      <Text color={account ? colors.text : colors.subText}>
                        {account || "Select"}
                      </Text>
                    </Pressable>
                  </View>
        ...
        <Box mt="$4" px="$4" py="$4" minHeight={400} bg={colors.card}>
          ...
          >
            ...
            <View style={{ flex: 1 }}>
              <FlatList
                data={[
                  ...currentCategories,
                  { category_id: "add-new", name: "ADD_NEW", icon: "➕" },
                ]}
                keyExtractor={(item) =>
                  item.category_id ? String(item.category_id) : item.name
                }
      ...
      >
        <View style={{ flex: 1, justifyContent: "flex-end" }}>
          ...
          >
            ...
            <View style={{ flex: 1 }}>
              <FlatList
                data={[...userAccounts, { name: "ADD_NEW", icon: "➕" }]}
                keyExtractor={(item) =>
                  String(item.account_id || item.id || item.name)
                }
                numColumns={NUM_COLUMNS}
                showsVerticalScrollIndicator={false}
                contentContainerStyle={{
  ...
  return (
    <Box>
      <HStack justifyContent="space-between" alignItems="center">
        <Text color={colors.subText} w={90}>
          {label}
        </Text>
